...
mod tests {
    ...
    fn test_cache_full() {
        ...
        assert!(cache.set("key3".to_string(), serde_json::json!("val3"), 60).is_err());
    }
    ...
    fn test_cache_boundary_conditions() {
        ...
        assert!(cache.set("key".to_string(), serde_json::json!("val"), 60).is_err());
        
        // Test with size 1
        let cache = Cache::new(1);
        assert!(cache.set("key1".to_string(), serde_json::json!("val1"), 60).is_ok());
        assert!(cache.set("key2".to_string(), serde_json::json!("val2"), 60).is_err());
    }
    ...
    fn test_cache_default_construction() {
        ...
        // 1001st should fail
        assert!(cache.set("overflow".to_string(), serde_json::json!("fail"), 60).is_err());
    }
