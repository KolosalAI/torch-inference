...
impl ProviderSummary {
    pub fn from_runs(
        ...
        let avg_ttft_ms = {
            let values: Vec<f64> = successes.iter().filter_map(|r| r.ttft_ms).collect();
            if values.is_empty() {
                None
            } else {
                Some(mean(&values))
...
async fn bench_local_tts(client: &Client, text: &str) -> RunResult {
        ...
        .await
    {
        ...
        Ok(r) if !r.status().is_success() => {
            ...
            RunResult::failure(
                ms(start),
                format!("HTTP {} — {}", status, trunc(&body, 200)),
            )
        }
        Ok(r) => {
...
async fn run_tts_benchmarks(client: &Client, runs: usize) -> Vec<ProviderSummary> {
    ...
    if probe_local(client).await {
        ...
        skip(
            "torch-inference (local)",
            "server not reachable at LOCAL_SERVER_URL",
        );
    }
...
fn print_table(summaries: &[ProviderSummary], category: &'static str, title: &str) {
    let rows: Vec<&ProviderSummary> = summaries
        .iter()
    ...
    if has_ttft {
        ...
        println!("  {}", "·".repeat(86));
        for r in &rows {
            let tok_s = if category == "text" {
                r.avg_throughput / 4.0
            } else {
                f64::NAN
            };
            println!(
    ...
    if category == "tts" {
        println!(
            "  chars/s = input_chars ÷ synthesis_time  (both providers receive the same text)"
        );
        println!(
            "  RTF = synthesis_time ÷ audio_duration  (lower is better; audio_duration ≈ chars/14)"
        );
        for r in &rows {
...
async fn main() {
    ...
    if do_image {
        all.extend(run_image_benchmarks(&client, runs).await);
        println!();
    }
    if do_text {
        all.extend(run_text_benchmarks(&client, runs).await);
        println!();
    }
    ...
    print_table(
        &all,
        "tts",
        "TTS SYNTHESIS  —  chars/sec of text synthesised to audio",
    );
    print_table(
        &all,
        "image",
        "IMAGE CLASSIFICATION  —  end-to-end latency per image",
    );
    print_table(
        &all,
        "text",
        "TEXT INFERENCE  —  chars/sec and tokens/sec (streaming)",
    );
...
mod tests {
    ...
    fn test_summary_success_rate_half_failed() {
        let results = vec![ok_run(100.0, 400), RunResult::failure(0.0, "timeout")];
        let s = ProviderSummary::from_runs("P", "m", "text", "chars/s", results);
    ...
    fn test_summary_throughput_chars_per_sec() {
        ...
        let s = ProviderSummary::from_runs("P", "m", "text", "chars/s", results);
        assert!(
            (s.avg_throughput - 4000.0).abs() < 1.0,
            "got {}",
            s.avg_throughput
        );
    }
