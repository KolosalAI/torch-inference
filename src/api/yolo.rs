...
use crate::core::yolo::{load_coco_names, YoloResults, YoloSize, YoloVersion};
...
pub async fn download_model(
    ...
    let version = YoloVersion::from_str(&req.model_version).ok_or_else(|| {
        ApiError::BadRequest(format!("Invalid YOLO version: {}", req.model_version))
    })?;
...
mod tests {
    use super::*;
    use crate::core::yolo::{YoloSize, YoloVersion};
    ...
    async fn test_detect_objects_model_exists_no_torch_returns_internal_error() {
        ...
        let tmp = std::env::temp_dir().join(format!("yolo_test_models_{}", uuid::Uuid::new_v4()));
        tokio::fs::create_dir_all(tmp.join("yolov8n"))
            .await
            .unwrap();
