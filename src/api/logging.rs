...
pub async fn get_logging_info() -> Result<HttpResponse, ApiError> {
    ...
    log::info!(
        "[ENDPOINT] Found {} log files",
        log_info.available_log_files.len()
    );
...
pub async fn get_log_file(
    ...
    log::info!(
        "[ENDPOINT] Log file requested: {}, lines: {}, from_end: {}",
        log_file,
        lines,
        from_end
    );
...
pub async fn clear_log_file(log_file: web::Path<String>) -> Result<HttpResponse, ApiError> {
    let log_file = log_file.into_inner();
    ...
    log::info!(
        "[ENDPOINT] Log file {} cleared successfully (was {} bytes)",
        log_file,
        original_size
    );
...
mod tests {
    ...
    async fn test_get_logging_info_returns_200() {
        let app =
            test::init_service(App::new().route("/logs", web::get().to(get_logging_info))).await;
    ...
    async fn test_get_log_file_invalid_name_returns_400() {
        let app =
            test::init_service(App::new().route("/logs/{log_file}", web::get().to(get_log_file)))
                .await;
    ...
    async fn test_clear_log_file_invalid_name_returns_400() {
        let app = test::init_service(
            App::new().route("/logs/{log_file}", web::delete().to(clear_log_file)),
        )
        .await;
