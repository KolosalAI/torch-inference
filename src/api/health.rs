...
pub async fn readiness(req: HttpRequest, monitor: web::Data<Arc<Monitor>>) -> Result<HttpResponse> {
    let correlation_id = get_correlation_id(&req);
...
pub async fn health(req: HttpRequest, monitor: web::Data<Arc<Monitor>>) -> Result<HttpResponse> {
    let correlation_id = get_correlation_id(&req);
    let start = Instant::now();
    let metrics = monitor.get_metrics();
    let health = monitor.get_health_status();
...
mod handler_tests {
    ...
    async fn test_liveness_handler_body_has_status() {
        ...
        assert!(
            ...
            "liveness response must have a status field"
        );
        assert!(
    ...
    async fn test_readiness_handler_body_has_checks() {
        ...
        assert!(
            ...
            "readiness response must have a checks object"
        );
        let checks = body["checks"].as_object().unwrap();
        assert!(
            checks.contains_key("error_rate"),
            "must have error_rate check"
        );
        assert!(
            checks.contains_key("queue_depth"),
            "must have queue_depth check"
        );
        assert!(
            checks.contains_key("response_time"),
            "must have response_time check"
        );
    }
    ...
    async fn test_health_handler_body_has_all_checks() {
        ...
        assert!(
            checks.contains_key("performance"),
            "must have performance check"
        );
    ...
    async fn test_readiness_with_requests_and_no_errors_computes_error_rate() {
        ...
        let body_bytes = actix_web::body::to_bytes(resp.into_body()).await.unwrap();
        let body: serde_json::Value = serde_json::from_slice(&body_bytes).unwrap();
        let error_rate_msg = body["checks"]["error_rate"]["message"]
            .as_str()
            .unwrap_or("");
        assert!(
            error_rate_msg.contains("0.00"),
