...
fn de_i32_or_str<'de, D: serde::Deserializer<'de>>(d: D) -> Result<i32, D::Error> {
    match serde_json::Value::deserialize(d)? {
        serde_json::Value::Number(n) => {
            Ok(n.as_i64().and_then(|v| i32::try_from(v).ok()).unwrap_or(0))
        }
...
pub async fn list_downloaded_models() -> Result<HttpResponse> {
    let registry = get_registry();
    let downloaded: HashMap<_, _> = registry
...
async fn download_model_async(model_id: &str, model: &ModelInfo) -> anyhow::Result<()> {
    ...
    if model.url.contains("resolve") {
        let extension = if model.url.ends_with(".pth") {
            "pth"
        } else if model.url.ends_with(".onnx") {
            "onnx"
        } else {
            "bin"
        };
        ...
        log::info!(
            "Successfully downloaded repository {} to {:?}",
            repo_id,
            cache_dir
        );
    } else if {
...
async fn download_huggingface_repo(
    repo_id: &str,
    ...
    if !response.status().is_success() {
        anyhow::bail!(
            "Failed to fetch repository file list: HTTP {}",
            response.status()
        );
    }
    ...
    let files_to_download: Vec<(String, std::path::PathBuf)> = files
        .iter()
        .filter_map(|f| {
            if f["type"] == "file" {
    ...
    log::info!(
        "Downloading {} files concurrently from repository",
...
mod handler_coverage_tests {
    ...
    async fn test_download_huggingface_repo_downloads_files() {
        ...
        let files: Vec<(String, std::path::PathBuf)> = vec![
            (
                format!("{}/model.onnx", file_server.uri()),
                dir.path().join("model.onnx"),
            ),
            (
                format!("{}/config.json", file_server.uri()),
    ...
    async fn test_download_model_async_resolve_url_with_config_note() {
        ...
        let _config_content = b"{\"key\": \"value\"}";
...
mod tests {
    ...
    async fn test_download_files_concurrent_tolerates_per_file_errors() {
        ...
        assert!(
            result.is_ok(),
            "per-file 404 should be tolerated as a warning"
        );
    }
    ...
    async fn test_download_model_async_builtin_returns_ok() {
        ...
        assert!(
            result.is_ok(),
            "built-in model should return Ok without downloading"
        );
    }
    ...
    // ── download_model_async – direct .pt URL (4th branch) ───────────────────
    ...
    // ── download_model_async – direct .pth URL (4th branch) ──────────────────
    ...
    async fn test_download_model_async_resolve_url_onnx_downloads_file() {
        ...
        assert!(
            result.is_ok(),
            "resolve URL onnx download should succeed: {:?}",
            result
        );
    ...
    async fn test_download_model_async_resolve_url_pth_downloads_file() {
        ...
        assert!(
            result.is_ok(),
            "resolve URL pth download should succeed: {:?}",
            result
        );
    ...
    async fn test_download_model_async_resolve_url_bin_extension() {
        ...
        assert!(
            result.is_ok(),
            "resolve URL bin download should succeed: {:?}",
            result
        );
    ...
    async fn test_download_huggingface_repo_enters_function_body() {
        ...
        let result =
            download_huggingface_repo("absolutely-nonexistent-org/no-such-repo", dir.path()).await;
    ...
    fn test_huggingface_repo_file_filter_logic() {
        ...
        assert_eq!(
            files_to_download.len(),
            2,
            "only model.onnx and config.json pass"
        );
        let paths: Vec<&str> = files_to_download
            .iter()
    ...
    async fn test_download_files_concurrent_dest_with_parent_creates_dir() {
        ...
        assert!(
            dest.exists(),
            "file should be written inside the created subdirectory"
        );
    }
}
...
mod model_info_tests {
    ...
    fn test_model_info_with_note() {
        ...
        assert_eq!(
            info.note,
            Some("https://config.example.com/config.json".to_string())
        );
    }
    ...
    fn test_de_string_or_num_null_becomes_empty_string() {
        ...
        assert_eq!(
            info.voices, "",
            "null voices should deserialize as empty string"
        );
    }
    ...
    fn test_de_string_or_num_bool_becomes_empty_string() {
        ...
        assert_eq!(
            info.voices, "",
            "boolean voices should deserialize as empty string"
        );
    }
    ...
    fn test_de_f32_or_str_null_becomes_zero() {
        ...
        let info: ModelInfo = serde_json::from_str(json).unwrap();
        assert!(
            (info.score - 0.0).abs() < 1e-6,
            "null score should become 0.0"
        );
    }
...
mod api_endpoint_tests {
    ...
    async fn list_downloaded_handler(
        data: web::Data<ModelRegistry>,
    ) -> actix_web::Result<HttpResponse> {
    ...
    async fn get_comparison_handler(
        data: web::Data<ModelRegistry>,
    ) -> actix_web::Result<HttpResponse> {
    ...
    async fn test_list_downloaded_endpoint_returns_only_downloaded() {
        let registry = make_test_registry();
        let app = test::init_service(App::new().app_data(web::Data::new(registry)).route(
...
mod registry_tests {
    ...
    fn test_load_falls_back_to_compiled_in_registry() {
        ...
        std::env::set_var(
            "MODEL_REGISTRY_PATH",
            "/this/path/does/not/exist/registry.json",
        );
        let registry = ModelRegistry::load();
        std::env::remove_var("MODEL_REGISTRY_PATH");
...
mod download_coverage_tests {
    ...
    async fn test_download_files_concurrent_mkdir_fails_returns_ok_overall() {
        ...
        assert!(
            result.is_ok(),
            "mkdir failure should be tolerated (returns Ok)"
        );
    }
    ...
    async fn test_download_model_async_resolve_url_no_note() {
        ...
        assert!(
            result.is_ok(),
            "resolve URL with no note should succeed: {:?}",
            result
        );
    ...
    async fn test_download_model_async_resolve_url_with_http_note_downloads_config() {
        ...
        let _config_content = b"{\"model\": \"config\"}";
    ...
    async fn test_download_model_async_resolve_url_note_plain_text_skips_config() {
        ...
        assert!(
            result.is_ok(),
            "note without http should skip config download: {:?}",
            result
        );
    ...
    async fn test_download_model_async_other_url_logs_manual_warning() {
        ...
        assert!(
            result.is_ok(),
            "manual download URL should return Ok: {:?}",
            result
        );
    ...
    async fn test_download_huggingface_repo_body_executes() {
        ...
        let result =
            download_huggingface_repo("nonexistent-org-xyz/nonexistent-repo-abc", dir.path()).await;
...
mod uncovered_lines_tests {
    ...
    async fn test_line_268_unknown_type_other_dir() {
        let model = ModelInfo {
            ...
            status: "Active".to_string(),
            note: None,
            model_type: "totally-unknown-xyz".to_string(),
            task: String::new(),
        };
    ...
    async fn test_lines_276_277_builtin_url_early_return() {
        ...
        assert!(
            result.is_ok(),
            "Built-in URL must return Ok immediately: {:?}",
            result
        );
    ...
    async fn test_line_284_bin_extension_arm() {
        ...
        assert!(
            result.is_ok(),
            "bin extension arm must succeed: {:?}",
            result
        );
    ...
    async fn test_line_313_manual_download_warning() {
        ...
        assert!(
            result.is_ok(),
            "manual download branch returns Ok: {:?}",
            result
        );
    ...
    async fn test_line_360_all_downloads_fail_warning() {
        ...
        assert!(
            result.is_ok(),
            "all-fail still returns Ok (line 360 must run)"
        );
    }
    ...
    async fn test_lines_386_387_388_hf_repo_file_filter_chain() {
        let dir = tempfile::tempdir().unwrap();
        let result =
            download_huggingface_repo("nonexistent-for-lines-386-388/repo", dir.path()).await;
        let _ = result;
    }
    ...
    async fn test_lines_207_208_spawn_task_logs_error_for_failing_download() {
        ...
        assert!(
            resp.status() == actix_web::http::StatusCode::OK
                || resp.status() == actix_web::http::StatusCode::NOT_FOUND,
            "response must be 200 or 404, got: {}",
            resp.status()
        );
