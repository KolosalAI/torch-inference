...
pub async fn profile_inference(
    ...
    let pre_metrics = ResourceMetrics {
        memory_mb: system
            ...
            .unwrap_or(0.0),
        cpu_percent: system.cpus().iter().map(|cpu| cpu.cpu_usage()).sum::<f32>()
    ...
    log::info!(
        "[ENDPOINT] Profiling completed in {:.2}ms",
...
pub async fn optimize_performance(
    ...
    log::info!(
        "[ENDPOINT] Optimization completed, freed {:.2} MB",
        memory_freed
    );
...
mod tests {
    ...
    async fn test_optimize_performance_returns_200() {
        ...
        let app = test::init_service(App::new().app_data(state.clone()).route(
            ...
            web::post().to(optimize_performance),
        ))
        .await;
        let req = test::TestRequest::post()
    ...
    async fn test_optimize_performance_optimizations_content() {
        ...
        let req = test::TestRequest::post()
            ...
            .to_request();
        let body: serde_json::Value = test::call_and_read_body_json(&app, req).await;
        let opts: Vec<&str> = body["optimizations_applied"]
            .as_array()
            .unwrap()
            .iter()
        ...
        assert!(opts
            .iter()
            .any(|s| s.contains("Memory") || s.contains("cleanup") || s.contains("cache")));
    }
    ...
    async fn test_profile_result_struct() {
        let pre = ResourceMetrics {
            memory_mb: 100.0,
            cpu_percent: 10.0,
        };
        let post = ResourceMetrics {
            memory_mb: 110.0,
            cpu_percent: 20.0,
        };
