...
pub async fn classify_image_file(
    ...
    match state.classifier.classify(&path, top_k) {
        Ok(results) => Ok(HttpResponse::Ok().json(ClassifyResponse {
            success: true,
            results: Some(results),
            error: None,
        })),
        Err(e) => Ok(HttpResponse::Ok().json(ClassifyResponse {
            success: false,
            results: None,
...
mod handler_body_tests {
    ...
    async fn test_hb_classify_file_success_branch_lines_44_48() {
        ...
        let classifier =
            Arc::new(ImageClassifier::new(model_p, labels, None, None).expect("classifier"));
        let state = web::Data::new(ImageClassificationState { classifier });
        ...
        let jpeg_bytes: &[u8] = &[
            ...
            0x01, 0x01, 0x00, 0x00, 0x3F, 0x00, 0xFB, 0x02, 0x8A, 0x28, 0x03, 0xFF, 0xD9,
        ];
        use std::io::Write;
    ...
    async fn test_hb_upload_success_branch_lines_104_108() {
        ...
        let jpeg_bytes: &[u8] = &[
            ...
            0x01, 0x00, 0x00, 0x3F, 0x00, 0xFB, 0x02, 0x8A, 0x28, 0x03, 0xFF, 0xD9,
        ];
