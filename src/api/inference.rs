...
use crate::core::neural_network::{InferenceResult, NetworkMetadata, NeuralNetwork};
...
pub async fn predict(
    ...
    match network.predict_from_slice(&req.input_data, &req.input_shape) {
        Ok(result) => Ok(HttpResponse::Ok().json(InferenceResponse {
            success: true,
            result: Some(result),
            error: None,
        })),
        Err(e) => Ok(HttpResponse::Ok().json(InferenceResponse {
            success: false,
            result: None,
...
pub async fn list_models(state: web::Data<NeuralNetworkState>) -> Result<HttpResponse, ApiError> {
    ...
    Ok(HttpResponse::Ok().json(ModelListResponse { models, total }))
}
...
mod configure_and_found_tests {
    ...
    async fn test_configure_registers_all_nn_routes() {
        ...
        let app =
            actix_test::init_service(App::new().app_data(state.clone()).configure(configure)).await;
    ...
    async fn test_unload_model_success_removes_network() {
        ...
        if !networks.is_empty() {
            ...
            assert_eq!(result.unwrap().status(), actix_web::http::StatusCode::OK);
            assert!(
                networks.get("to-unload").is_none(),
