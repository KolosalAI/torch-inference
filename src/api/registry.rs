...
pub async fn register_model(
    ...
    match engine
        .model_manager
        .register_model_from_path(&path, req.name.clone())
        .await
    {
        Ok(model_id) => {
            let metadata = engine
...
pub async fn infer_pytorch(
    ...
    #[cfg(feature = "torch")]
    {
        match engine
            .model_manager
            .infer_pytorch(&req.model_id, &req.input)
            .await
        {
            Ok(output) => Ok(
                HttpResponse::Ok().json(ApiResponse::success(serde_json::json!({
...
/// Export registry
pub async fn export_registry(engine: web::Data<Arc<InferenceEngine>>) -> ActixResult<HttpResponse> {
    ...
    cfg.service(
        web::scope("/registry")
            ...
            .route(
                ...
                web::post().to(load_pytorch_model),
            )
...
mod tests {
    ...
    fn test_unknown_format_string_is_recognized() {
        ...
        let is_unknown = !matches!(
            format_str.to_lowercase().as_str(),
    ...
    async fn test_list_registered_models_handler_with_onnx_format() {
        let engine = make_engine();
        let query = web::Query(ListByFormatQuery {
    ...
    async fn test_list_registered_models_handler_unknown_format() {
        let engine = make_engine();
        let query = web::Query(ListByFormatQuery {
            format: Some("exotic_format".to_string()),
        });
        let resp = list_registered_models(engine, query).await.unwrap();
    ...
    async fn test_get_model_metadata_handler_success() {
        ...
        let manager = Arc::new(crate::models::manager::ModelManager::new(
        ...
        let engine = Arc::new(crate::core::engine::InferenceEngine::new(
            manager,
            &crate::config::Config::default(),
        ));
