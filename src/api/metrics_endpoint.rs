...
pub async fn metrics_handler() -> HttpResponse {
    match prometheus::render_metrics() {
        Ok(metrics_text) => HttpResponse::Ok()
            ...
            .body(metrics_text),
        Err(e) => {
            HttpResponse::InternalServerError().body(format!("Failed to render metrics: {}", e))
        }
    }
}
...
mod tests {
    ...
    async fn test_metrics_endpoint_content_type_is_plain_text() {
        ...
        let app =
            actix_test::init_service(App::new().route("/metrics", web::get().to(metrics_handler)))
                .await;
    ...
    async fn test_metrics_endpoint_body_is_string() {
        ...
        let body_str =
            std::str::from_utf8(&body_bytes).expect("metrics body should be valid UTF-8");
