...
pub async fn synthesize(
    ...
    if req.text.len() > 50000 {
        return Err(ApiError::BadRequest(
            "Text too long (max 50000 characters)".to_string(),
        ));
    }
...
pub async fn list_engines(state: web::Data<TTSState>) -> Result<HttpResponse, ApiError> {
...
pub async fn get_stats(state: web::Data<TTSState>) -> Result<HttpResponse, ApiError> {
    let stats = state.manager.get_stats();
    Ok(HttpResponse::Ok().json(StatsResponse { stats }))
}

/// GET /tts/health - Health check
pub async fn health_check(state: web::Data<TTSState>) -> Result<HttpResponse, ApiError> {
...
mod tests {
    ...
    use crate::core::tts_engine::{
        EngineCapabilities, SynthesisParams, VoiceGender, VoiceInfo, VoiceQuality,
    };
    ...
    fn make_tts_state(engine_id: &str) -> web::Data<TTSState> {
        ...
        manager
            .register_engine(engine_id.to_string(), Arc::new(MockTTSEngine::new()))
            .unwrap();
    ...
    async fn test_synthesize_handler_empty_text() {
        ...
        let config = web::Data::new(crate::config::Config::default());
        let http_req = actix_web::test::TestRequest::default().to_http_request();
        let result = synthesize(req, state, config, http_req).await;
    ...
    async fn test_synthesize_handler_success() {
        ...
        let config = web::Data::new(crate::config::Config::default());
        let http_req = actix_web::test::TestRequest::default().to_http_request();
        let resp = synthesize(req, state, config, http_req).await.unwrap();
        assert_eq!(resp.status(), actix_web::http::StatusCode::OK);
    }
    ...
    #[actix_web::test]
    async fn test_synthesize_handler_envelope_meta_has_required_fields() {
        let state = make_tts_state("mock-tts");
        let req = web::Json(SynthesisRequest {
            text: "hello".to_string(),
            engine: Some("mock-tts".to_string()),
    ...
    #[actix_web::test]
    async fn test_synthesize_handler_skip_postprocess_sets_meta_applied_false() {
        let state = make_tts_state("mock-tts");
        let req = web::Json(SynthesisRequest {
            text: "hello".to_string(),
            engine: Some("mock-tts".to_string()),
    ...
    // stream_synthesize — empty text returns BadRequest
    #[actix_web::test]
    async fn stream_handler_rejects_empty_text() {
        let state = make_tts_state("mock-tts");
        let req = web::Json(SynthesisRequest {
            text: "".to_string(),
            engine: None,
    ...
    // stream_synthesize — text longer than 50000 chars returns BadRequest
    #[actix_web::test]
    async fn stream_handler_rejects_oversized_text() {
        let state = make_tts_state("mock-tts");
        let req = web::Json(SynthesisRequest {
            text: "a".repeat(50001),
            engine: None,
...
pub fn configure_routes(cfg: &mut web::ServiceConfig) {
    cfg.service(
        web::scope("/tts")
            ...
            .route(
                "/engines/{engine_id}/capabilities",
