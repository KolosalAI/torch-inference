...
pub async fn synthesize_speech(
    ...
    // Sanitize text
    let sanitized_text = state
        .sanitizer
        .sanitize_input(&serde_json::json!(req.text))
...
pub async fn validate_audio(mut payload: Multipart) -> Result<HttpResponse, ApiError> {
    ...
    match processor.validate_audio(&audio_data) {
        Ok(metadata) => Ok(HttpResponse::Ok().json(AudioValidationResponse {
            valid: true,
...
mod tests {
    ...
    async fn test_synthesize_request_serde_all_fields() {
        let json =
            r#"{"text":"hello","model":"default","voice":"speaker1","speed":1.2,"pitch":0.9}"#;
        let mut buf = json.as_bytes().to_vec();
        let req: SynthesizeRequest = simd_json::from_slice(&mut buf).unwrap();
    ...
    async fn test_synthesize_response_serialization() {
        ...
        let mut buf = json.as_bytes().to_vec();
        let back: serde_json::Value = simd_json::from_slice(&mut buf).unwrap();
    ...
    async fn test_transcribe_request_serde() {
        ...
        let mut buf = json.as_bytes().to_vec();
        let req: TranscribeRequest = simd_json::from_slice(&mut buf).unwrap();
    ...
    async fn make_audio_state_with_tts() -> web::Data<AudioState> {
        use crate::config::SanitizerConfig;
        use crate::core::audio_models::{AudioModelManager, TTSConfig};
    ...
    async fn make_audio_state_with_stt() -> web::Data<AudioState> {
        use crate::config::SanitizerConfig;
        use crate::core::audio_models::{AudioModelManager, STTConfig};
        use std::path::PathBuf;
    ...
    async fn test_synthesize_speech_success_returns_200() {
        ...
        assert!(body["data"]["audio_base64"].as_str().is_some());
        assert_eq!(body["data"]["format"], "wav");
        assert!(body["data"]["sample_rate"].as_u64().unwrap() > 0);
        assert!(body["data"]["duration_secs"].as_f64().unwrap() > 0.0);
    }
    ...
    async fn test_synthesize_speech_audio_base64_decodes_to_wav() {
        ...
        let decoded = general_purpose::STANDARD
            .decode(body["data"]["audio_base64"].as_str().unwrap())
            .unwrap();
        assert!(decoded.len() > 4);
        assert_eq!(
            &decoded[0..4],
            b"RIFF",
            "decoded audio should be a WAV RIFF file"
        );
    }
    ...
    async fn test_validate_audio_empty_field_body_returns_valid_false() {
        ...
        assert!(errors
            .iter()
            .any(|e| e.as_str().unwrap_or("").contains("No audio data provided")));
    }

    // ── Envelope / postprocessing meta ───────────────────────────────────────

    #[tokio::test]
    async fn test_synthesize_speech_envelope_meta_has_required_fields() {
        let state = make_audio_state_with_tts().await;
        let app = test::init_service(
            App::new()
                .app_data(state)
        ...
        let body: serde_json::Value = test::call_and_read_body_json(
            ...
            test::TestRequest::post()
                .uri("/audio/synthesize")
                .set_json(&serde_json::json!({"text": "hello", "model": "default"}))
                .to_request(),
        )
        .await;
        let meta = &body["meta"];
    ...
    #[tokio::test]
    async fn test_synthesize_speech_skip_postprocess_sets_meta_applied_false() {
        let state = make_audio_state_with_tts().await;
        let app = test::init_service(
            App::new()
                .app_data(state)
        ...
        let body: serde_json::Value = test::call_and_read_body_json(
            ...
            test::TestRequest::post()
                .uri("/audio/synthesize")
                .set_json(&serde_json::json!({"text": "hello", "model": "default", "skip_postprocess": true}))
                .to_request(),
        ).await;
...
mod simd_json_tests {
    ...
    fn simd_json_roundtrip_matches_serde() {
        let json =
            r#"{"text":"hello world","model":"kokoro","voice":"af_bella","speed":1.0,"pitch":0.9}"#;
        let expected: SynthesizeRequest = serde_json::from_str(json).unwrap();
        let mut buf = json.as_bytes().to_vec();
        let actual: SynthesizeRequest = simd_json::from_slice(&mut buf).unwrap();
        assert_eq!(expected.text, actual.text);
        assert_eq!(expected.model, actual.model);
        assert_eq!(expected.voice, actual.voice);
        assert_eq!(expected.speed, actual.speed);
        assert_eq!(expected.pitch, actual.pitch);
    }
}
