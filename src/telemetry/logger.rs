...
pub(crate) fn format_log_record_inner(
    ...
    writeln!(
        ...
        timestamp, level_string, short_target, args
    )
}
...
mod tests {
    ...
    fn test_format_log_record_inner_strips_torch_inference_prefix() {
        ...
        format_log_record_inner(
            &mut buf,
            log::Level::Info,
            "torch_inference::api::foo",
            &args,
        )
        .unwrap();
        let out = String::from_utf8(buf).unwrap();
        ...
        assert!(
            !out.contains("torch_inference::api::foo"),
            "full prefix should not appear"
        );
    }
    ...
    fn test_format_log_record_inner_no_strip_for_other_crate() {
        ...
        let out = String::from_utf8(buf).unwrap();
        assert!(
            out.contains("other_crate::mod"),
            "non-torch target should not be stripped"
        );
    }
