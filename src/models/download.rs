...
impl ModelDownloadManager {
    ...
    async fn execute_download(&self, task_id: String) -> Result<()> {
        ...
        let task = self.tasks.get(&task_id).context("Task not found")?.clone();
        ...
        match &task.source {
            ModelSource::HuggingFace { repo_id, revision } => {
                self.download_from_huggingface(&task_id, repo_id, revision.as_deref(), &model_dir)
                    .await?;
            }
            ModelSource::Url { url } => {
    ...
    async fn download_from_huggingface(
        ...
        log::info!(
            "Downloading from HuggingFace: {} (revision: {})",
            repo_id,
            revision
        );
                ...
                async move {
                    ...
                    while let Some(chunk) = stream.next().await {
                        ...
                        if let Some(total) = total_size {
                            let chunk_progress = (downloaded as f32 / total as f32)
                                * (1.0 / total_files as f32)
                                * 100.0;
                            mgr.update_task_progress(
                                &tid,
                                file_progress + chunk_progress,
                                downloaded,
                                total_size,
                            );
                        }
    ...
    #[cfg(test)]
    pub async fn download_from_url_pub(
        &self,
        task_id: &str,
        url: &str,
        target_dir: &Path,
    ...
    pub async fn delete_model(&self, name: &str) -> Result<()> {
        if let Some((_, model)) = self.models.remove(name) {
            if model.local_path.exists() {
                fs::remove_dir_all(&model.local_path)
                    .await
...
mod tests {
    ...
    fn test_download_status_serde_roundtrip() {
        ...
        for status in &statuses {
            ...
            let deserialized: DownloadStatus = simd_json::from_slice(&mut buf).unwrap();
    ...
    fn test_model_source_serde_roundtrip_huggingface() {
        ...
        let mut buf = json.as_bytes().to_vec();
        let deserialized: ModelSource = simd_json::from_slice(&mut buf).unwrap();
        match deserialized {
            ModelSource::HuggingFace { repo_id, revision } => {
    ...
    fn test_model_source_serde_roundtrip_url() {
        let source = ModelSource::Url {
            url: "http://example.com/m.bin".to_string(),
        };
        let json = serde_json::to_string(&source).unwrap();
        let mut buf = json.as_bytes().to_vec();
        let deserialized: ModelSource = simd_json::from_slice(&mut buf).unwrap();
    ...
    fn test_model_metadata_serde_roundtrip() {
        ...
        let mut buf = json.as_bytes().to_vec();
        let deserialized: ModelMetadata = simd_json::from_slice(&mut buf).unwrap();
    ...
    fn test_model_metadata_serde_with_explicit_tags() {
        ...
        let json = r#"{"tags":[]}"#;
        let mut buf = json.as_bytes().to_vec();
        let meta: ModelMetadata = simd_json::from_slice(&mut buf).unwrap();
        assert!(meta.description.is_none());
        assert!(meta.tags.is_empty());
    }
    ...
    fn test_download_task_serde_roundtrip() {
        ...
        let mut buf = json.as_bytes().to_vec();
        let deserialized: DownloadTask = simd_json::from_slice(&mut buf).unwrap();
    ...
    fn test_model_info_serde_roundtrip() {
        ...
        let mut buf = json.as_bytes().to_vec();
        let deserialized: ModelInfo = simd_json::from_slice(&mut buf).unwrap();
    ...
    fn test_cache_info_serde_roundtrip() {
        ...
        let mut buf = json.as_bytes().to_vec();
        let deserialized: CacheInfo = simd_json::from_slice(&mut buf).unwrap();
    ...
    fn test_manager_clone_shares_state() {
        ...
        assert_eq!(
            manager.max_concurrent_downloads,
            cloned.max_concurrent_downloads
        );
    }
    ...
    fn test_manager_get_model_found() {
        ...
        let model = ModelInfo {
            ...
            source: ModelSource::Local {
                path: "/tmp/found_model".to_string(),
            },
    ...
    async fn test_manager_delete_model_without_local_dir() {
        ...
        let model = ModelInfo {
            ...
            source: ModelSource::Local {
                path: "/tmp/nonexistent_ghost_model".to_string(),
            },
    ...
    fn test_manager_update_task_status_all_transitions() {
        ...
        manager
            .tasks
            .insert("transition-task".to_string(), base_task);

        // Pending -> Downloading
        manager.update_task_status("transition-task", DownloadStatus::Downloading);
        assert_eq!(
            manager.get_task_status("transition-task").unwrap().status,
            DownloadStatus::Downloading
        );
    ...
    fn test_manager_clone_shares_models_map() {
        ...
        let model = ModelInfo {
            ...
            source: ModelSource::Local {
                path: "/tmp/shared_model".to_string(),
            },
    ...
    async fn test_manager_delete_model_with_existing_dir() {
        ...
        tokio::fs::write(model_dir.join("weights.bin"), b"fake weights")
            .await
            .unwrap();
        ...
        assert!(
            !model_dir.exists(),
            "model directory should have been removed"
        );
        assert!(
            manager.get_model("my_model").is_none(),
    ...
    async fn test_manager_initialize_scans_existing_subdirs() {
        ...
        tokio::fs::create_dir_all(&model_dir).await.unwrap();
        tokio::fs::write(model_dir.join("pytorch_model.bin"), b"fake")
            .await
            .unwrap();
        ...
        assert!(
            manager.get_model("bert-base").is_some(),
    ...
    async fn test_manager_initialize_scans_subdir_with_metadata() {
        ...
        tokio::fs::write(model_dir.join("metadata.json"), meta_json.as_bytes())
            .await
            .unwrap();
        ...
        assert!(
            model_info.is_some(),
            "model with metadata should be registered"
        );
        let model_info = model_info.unwrap();
        assert_eq!(
            model_info.metadata.description.as_deref(),
            Some("GPT-2 small")
        );
    ...
    async fn test_manager_get_cache_info_after_initialize() {
        ...
        tokio::fs::create_dir_all(&model_dir).await.unwrap();
        tokio::fs::write(model_dir.join("file.bin"), b"hello world")
            .await
            .unwrap();
        ...
        assert!(
            ...
            "should report non-zero size after scanning"
        );
    ...
    fn test_download_task_all_status_variants_roundtrip() {
        ...
        for status in &statuses {
            ...
            let json = serde_json::to_string(&task).unwrap();
            let mut buf = json.as_bytes().to_vec();
            let deser: DownloadTask = simd_json::from_slice(&mut buf).unwrap();
    ...
    fn test_model_source_all_variants_debug() {
        let sources = vec![
            ...
            ModelSource::Url {
                url: "http://x.com/m.bin".to_string(),
            },
            ModelSource::Local {
    ...
    async fn test_scan_cache_nonexistent_dir_early_return() {
        ...
        assert!(
            result.is_ok(),
            "scan_cache on nonexistent dir should return Ok(())"
        );
    ...
    async fn test_scan_cache_reads_metadata_json_directly() {
        ...
        tokio::fs::write(model_dir.join("metadata.json"), meta_json.as_bytes())
            .await
            .unwrap();
        tokio::fs::write(model_dir.join("model.bin"), b"weights")
            .await
            .unwrap();
    ...
    async fn test_scan_cache_invalid_metadata_json_falls_back_to_default() {
        ...
        tokio::fs::create_dir_all(&model_dir).await.unwrap();
        tokio::fs::write(model_dir.join("metadata.json"), b"NOT { valid JSON }")
            .await
            .unwrap();
        tokio::fs::write(model_dir.join("weights.bin"), b"data")
            .await
            .unwrap();
    ...
    async fn test_calculate_dir_size_nested_subdir_recursion() {
        ...
        tokio::fs::write(root.join("top.bin"), b"abcd")
            .await
            .unwrap(); // 4 bytes
        tokio::fs::write(sub.join("deep.bin"), b"efghij")
            .await
            .unwrap(); // 6 bytes
        tokio::fs::write(sub2.join("deeper.bin"), b"xyz")
            .await
            .unwrap(); // 3 bytes
    ...
    async fn test_execute_download_url_success_covers_214_217_229() {
        ...
        assert!(
            result.is_ok(),
            "URL execute_download should succeed: {:?}",
            result.err()
        );
        ...
        assert!(
            model.is_some(),
            "model should be registered after success path"
        );
    ...
    async fn test_execute_download_url_non_2xx_response() {
        ...
        assert!(
            result.is_err(),
            "404 response should cause execute_download to fail"
        );
    ...
    async fn test_execute_download_url_progress_with_content_length() {
        ...
        assert!(
            result.is_ok(),
            "progress download should succeed: {:?}",
            result.err()
        );
    ...
    async fn test_scan_cache_empty_dir_produces_no_models() {
        ...
        manager.initialize().await.unwrap();
        assert!(
            manager.list_models().is_empty(),
            "empty cache dir should produce no models"
        );
    }
    ...
    async fn test_scan_cache_with_model_dir_no_metadata() {
        let tmp = tempfile::tempdir().unwrap();
        tokio::fs::create_dir(tmp.path().join("bert-base"))
            .await
            .unwrap();
        let manager = ModelDownloadManager::new(tmp.path()).unwrap();
        manager.initialize().await.unwrap();
    ...
    async fn test_concurrent_downloads_limited_by_semaphore() {
        ...
        let handles: Vec<_> = task_ids
            ...
            .map(|id| {
                let mgr = manager.clone();
                tokio::spawn(async move {
                    let _ = mgr.execute_download_pub(id).await;
                })
            })
            .collect();
        for h in handles {
    ...
    async fn test_scan_cache_with_metadata_and_file() {
        ...
        tokio::fs::write(model_dir.join("metadata.json"), meta.to_string())
            .await
            .unwrap();
        tokio::fs::write(model_dir.join("model.bin"), b"fake content")
            .await
            .unwrap();
        let manager = ModelDownloadManager::new(tmp.path()).unwrap();
        manager.initialize().await.unwrap();
        ...
        assert!(
            info.size_bytes > 0,
            "size_bytes should be > 0 after calculate_dir_size"
        );
