...
use crate::models::registry::{ModelMetadata, PostprocessingConfig, PreprocessingConfig};
...
impl PyTorchModelLoader {
    ...
    pub fn load_model(
        ...
        info!(
            "Loading PyTorch model from: {:?} (device override: {:?})",
        ...
        let model = match CModule::load_on_device(path, device) {
            ...
            Err(e) => {
                warn!(
                    "Failed to load as TorchScript: {}, trying as tensor checkpoint",
                    e
                );
    ...
    pub fn load_model(&self, _path: &Path) -> Result<LoadedPyTorchModel> {
        Err(anyhow::anyhow!(
            "PyTorch support not enabled. Compile with --features torch"
        ))
    }
    ...
    pub fn preprocess(
        &self,
        input: &Value,
        config: &Option<PreprocessingConfig>,
    ...
    fn preprocess_image(
        &self,
        data: &Value,
        config: &crate::models::registry::ImagePreprocessing,
    ) -> Result<PreprocessedInput> {
    ...
    fn preprocess_audio(
        &self,
        data: &Value,
        config: &crate::models::registry::AudioPreprocessing,
    ) -> Result<PreprocessedInput> {
    ...
    fn preprocess_text(
        &self,
        data: &Value,
        config: &crate::models::registry::TextPreprocessing,
    ) -> Result<PreprocessedInput> {
    ...
    pub fn postprocess(
        &self,
        output: ModelOutput,
        config: &Option<PostprocessingConfig>,
    ) -> Result<Value> {
        if let Some(cfg) = config {
    ...
    pub fn infer(
        ...
        let output = match &model.model {
            ...
            LoadedModel::Tensors(_tensors) => {
                ...
                return Err(anyhow::anyhow!(
                    "Raw tensor checkpoints require custom inference logic"
                ));
            }
        };
...
mod tests {
    ...
    fn test_pytorch_loader_none_device_defaults_to_cpu() {
        ...
        assert_eq!(
            loader.device, "cpu",
            "default device without torch feature must be 'cpu'"
        );
    }
    ...
    fn test_to_tensor_returns_err_without_torch_raw() {
        ...
        assert!(
            result.is_err(),
            "to_tensor should error without torch feature"
        );
    ...
    fn test_postprocess_detection_threshold_override() {
        ...
        assert!(
            (threshold - 0.7).abs() < 1e-5,
            "expected threshold ~0.7, got {}",
            threshold
        );
    }
