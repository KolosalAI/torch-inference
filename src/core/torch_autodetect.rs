...
mod tests {
    ...
    fn test_check_existing_installation_nonexistent_returns_none() {
        ...
        if let Some(v) = old_libtorch { env::set_var("LIBTORCH", v); }
    }
    ...
    fn test_detect_backend_cuda_library_present() {
        let tmpdir = tempfile::TempDir::new().unwrap();
        std::fs::create_dir_all(tmpdir.path().join("lib")).unwrap();
        std::fs::write(tmpdir.path().join("lib").join("libtorch_cuda.so"), b"fake cuda").unwrap();
    ...
    fn test_detect_backend_metal_library_present() {
        let tmpdir = tempfile::TempDir::new().unwrap();
        std::fs::create_dir_all(tmpdir.path().join("lib")).unwrap();
        std::fs::write(tmpdir.path().join("lib").join("libtorch_mps.dylib"), b"fake mps").unwrap();
    ...
    fn test_detect_cuda_via_versioned_cuda_path_var_nonexistent() {
        ...
        if let Some(v) = old { env::set_var("CUDA_PATH_V12_1", v); }
    ...
    fn test_detect_cuda_via_versioned_cuda_path_var_existing() {
        ...
        if let Some(v) = old_versioned { env::set_var("CUDA_PATH_V11_8", v); }
        if let Some(v) = old_base { env::set_var("CUDA_PATH", v); } else { env::remove_var("CUDA_PATH"); }
    }
    ...
    async fn test_initialize_torch_with_existing_install() {
        ...
        assert!(result.is_ok(), "initialize_torch should use existing install");
    ...
    async fn test_auto_setup_returns_existing_config_without_download() {
        ...
        assert!(result.is_ok(), "auto_setup should use existing install: {:?}", result.err());
    ...
    fn test_detect_cuda_path_exists_no_version_falls_through() {
        ...
        env::remove_var("CUDA_PATH_V12_0");
        if let Some(v) = saved_v12_0 { env::set_var("CUDA_PATH_V12_0", v); }
        if let Some(v) = old { env::set_var("CUDA_PATH", v); } else { env::remove_var("CUDA_PATH"); }
    }
    ...
    fn test_detect_metal_x86_64_exercises_framework_check() {
        ...
        let metal_fw_exists = std::path::Path::new(
    ...
    fn test_detect_cuda_nvidia_smi_success_covers_lines_153_157() {
        ...
        let versioned_vars: Vec<String> = (10u32..=13).flat_map(|maj| {
            (0u32..=9).map(move |min| format!("CUDA_PATH_V{}_{}", maj, min))
        }).collect();
        ...
        if let Some(v) = old_cuda_path { env::set_var("CUDA_PATH", v); } else { env::remove_var("CUDA_PATH"); }
        for (k, v) in saved_versioned {
            if let Some(val) = v { env::set_var(&k, val); } else { env::remove_var(&k); }
        }
    }
    ...
    fn test_detect_backend_cuda_so_present_with_no_cuda_version_uses_fallback_non_windows() {
        ...
        if let TorchBackend::Cuda(ver) = &backend {
            assert_eq!(ver, "11.8", "Expected fallback version 11.8 when cuda_version is None");
        }
    }
    ...
    async fn test_download_libtorch_covers_lines_341_to_358_via_proxy_failure() {
        ...
        if let Some(v) = old_https_proxy { env::set_var("HTTPS_PROXY", v); } else { env::remove_var("HTTPS_PROXY"); }
        if let Some(v) = old_https_proxy_lc { env::set_var("https_proxy", v); } else { env::remove_var("https_proxy"); }
    ...
    async fn test_download_libtorch_covers_lines_341_to_354_before_network() {
        ...
        assert!(result.is_err(), "expected Err from download_libtorch with broken proxy");
    }
    ...
    async fn test_download_libtorch_content_length_unwrap_or_zero_does_not_panic() {
        ...
        Mock::given(method("GET"))
            .respond_with(
                ResponseTemplate::new(200)
                    .set_body_bytes(b"some bytes".as_ref()),
            )
            .mount(&mock_server)
            .await;
