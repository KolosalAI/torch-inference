...
use crate::core::llm::sampler::{sample, softmax, SamplingParams};
...
impl SpeculativeDecoder {
    ...
    pub fn step(
        ...
        for (i, &dt) in draft_tokens.iter().enumerate() {
            ...
            let accept = acceptance_roll(alpha, i as u64);
            if accept {
                ...
                let corrected = sample(&logits_from_probs(&adjusted), &SamplingParams::greedy())?;
        ...
        Ok(SpecStepOutput { tokens: accepted_tokens, num_accepted })
    }
}

// ── Helpers ───────────────────────────────────────────────────────────────

/// Deterministic pseudo-acceptance roll (for testing reproducibility).
fn acceptance_roll(alpha: f32, seed: u64) -> bool {
    // Deterministic hash-based uniform sample in [0, 1).
    use std::collections::hash_map::DefaultHasher;
    use std::hash::{Hash, Hasher};
    let mut h = DefaultHasher::new();
    seed.hash(&mut h);
    let u = (h.finish() as f32) / (u64::MAX as f32);
    let u = u.abs();
    u < alpha
}
...
mod tests {
    ...
    impl DraftModel for MockDraftAlwaysZero {
        fn propose(&self, _ctx: &[u32], n: usize) -> Result<(Vec<u32>, Vec<Vec<f32>>)> {
            ...
            for row in &mut probs { row[0] = 1.0; }
            Ok((tokens, probs))
        }
    }
    ...
    impl TargetModel for MockTargetPerfectMatch {
        fn forward(&self, tokens: &[u32]) -> Result<Vec<Vec<f32>>> {
            ...
            for row in &mut logits { row[0] = 100.0; }
            Ok(logits)
        }
    }
    ...
    impl TargetModel for MockTargetAlwaysOne {
        fn forward(&self, tokens: &[u32]) -> Result<Vec<Vec<f32>>> {
            let mut logits = vec![vec![-100.0f32; VOCAB]; tokens.len()];
            for row in &mut logits { row[1] = 100.0; }
            Ok(logits)
        }
    }
    ...
    fn test_spec_step_output_fields() {
        let out = SpecStepOutput { tokens: vec![1, 2], num_accepted: 1 };
    ...
    fn test_acceptance_roll_always_rejects_zero_alpha() {
        assert!(!acceptance_roll(0.0, 0));
        assert!(!acceptance_roll(0.0, 42));
    }

    #[test]
    fn test_acceptance_roll_always_accepts_one_alpha() {
        // alpha = 1.0 → any u < 1.0 accepts
        assert!(acceptance_roll(1.0, 0));
        assert!(acceptance_roll(1.0, 999));
    }
