...
pub struct SchedulerOutput {
    ...
    /// Shared via Arc — cloning is O(1) and the inner Vec is mutated
    /// via Arc::make_mut only when the previous output has been dropped.
    pub running_seq_ids: Arc<Vec<SequenceId>>,
...
pub struct Scheduler {
    ...
    running: Arc<Vec<SequenceId>>,
    /// `AHashMap` (ahash) is ~2× faster than `std::HashMap` for `u64` keys
    /// because its hasher has a lower constant than SipHash-1-3.
    sequences: AHashMap<SequenceId, Sequence>,
    next_id: SequenceId,
}
...
impl Scheduler {
    ...
    pub fn schedule(&mut self) -> SchedulerOutput {
        ...
        let cap = max_new.min(self.waiting.len());
        let mut prefill_seq_ids: Vec<SequenceId> = Vec::with_capacity(cap);
        let mut prefill_set: AHashSet<SequenceId> = AHashSet::with_capacity(cap);
    ...
    pub fn drain_finished(&mut self) -> Vec<SequenceId> {
        ...
        Arc::make_mut(&mut self.running).retain(|&id| {
            let is_done = self
                .sequences
                .get(&id)
