...
mod tests {
    ...
    fn make_tiny_png(width: u32, height: u32, r: u8, g: u8, b: u8) -> Vec<u8> {
        let buf: ImageBuffer<Rgb<u8>, Vec<u8>> = ImageBuffer::from_pixel(width, height, Rgb([r, g, b]));
    ...
    fn make_tiny_rgba_png(width: u32, height: u32) -> Vec<u8> {
        let buf: ImageBuffer<Rgba<u8>, Vec<u8>> = ImageBuffer::from_pixel(width, height, Rgba([100, 150, 200, 255]));
    ...
    fn make_dyn_rgba_image(width: u32, height: u32) -> DynamicImage {
        let buf: ImageBuffer<Rgba<u8>, Vec<u8>> = ImageBuffer::from_pixel(width, height, Rgba([10, 20, 30, 255]));
    ...
    fn test_security_level_serde_roundtrip() {
        for level in [SecurityLevel::Low, SecurityLevel::Medium, SecurityLevel::High, SecurityLevel::Maximum] {
    ...
    fn test_check_adversarial_patterns_uniform_image_no_threat() {
        ...
        let adversarial_threats: Vec<_> = result.threats_detected.iter()
            .filter(|t| matches!(t.threat_type, ThreatType::AdversarialPattern))
            .collect();
        assert!(adversarial_threats.is_empty(), "uniform image should not trigger adversarial threat");
    }
    ...
    fn test_check_steganography_uniform_image_low_level() {
        ...
        let result = validator.validate(&png, SecurityLevel::Low).unwrap();
        let steg_threats: Vec<_> = result.threats_detected.iter()
        ...
        assert!(!steg_threats.is_empty(), "pure-black image should trigger steganography check");
    }
    ...
    fn test_check_metadata_no_exif_no_threat() {
        ...
        let meta_threats: Vec<_> = result.threats_detected.iter()
            .filter(|t| matches!(t.threat_type, ThreatType::SuspiciousMetadata))
            .collect();
        assert!(meta_threats.is_empty(), "plain PNG should not have metadata threat");
    }
    ...
    fn test_validate_valid_image_medium() {
        ...
        let fmt_threats: Vec<_> = result.threats_detected.iter()
    ...
    fn test_validate_invalid_format() {
        ...
        assert!(!result.is_safe);
        let has_invalid = result.threats_detected.iter()
    ...
    fn test_validate_excessive_file_size() {
        ...
        let result = validator.validate(&png, SecurityLevel::Low).unwrap();
        let has_excessive = result.threats_detected.iter()
            .any(|t| matches!(t.threat_type, ThreatType::ExcessiveSize));
        assert!(has_excessive, "file exceeding size limit should trigger ExcessiveSize threat");
    }
    ...
    fn test_validate_excessive_dimensions() {
        ...
        let has_excessive = result.threats_detected.iter()
            .any(|t| matches!(t.threat_type, ThreatType::ExcessiveSize));
        assert!(has_excessive, "image exceeding dimension limit should trigger ExcessiveSize threat");
    }
    ...
    fn test_validate_all_security_levels() {
        ...
        for level in [SecurityLevel::Low, SecurityLevel::Medium, SecurityLevel::High, SecurityLevel::Maximum] {
            ...
            assert!(result.is_ok(), "validate should not error for a valid PNG at any security level");
        }
    }
    ...
    fn test_get_image_stats_luma8_fallback() {
        ...
        let buf: ImageBuffer<Luma<u8>, Vec<u8>> =
            ImageBuffer::from_pixel(4, 4, Luma([128u8]));
    ...
    fn test_check_adversarial_patterns_maximum_no_threat_uniform() {
        ...
        let adversarial: Vec<_> = result.threats_detected.iter()
            .filter(|t| matches!(t.threat_type, ThreatType::AdversarialPattern))
            .collect();
    ...
    fn test_check_steganography_maximum_level_detects_deviation() {
        ...
        let png = make_tiny_png(4, 4, 0, 0, 0);
        let result = validator.validate(&png, SecurityLevel::Maximum).unwrap();
        let steg: Vec<_> = result.threats_detected.iter()
            .filter(|t| matches!(t.threat_type, ThreatType::Steganography))
            .collect();
    ...
    fn test_validate_is_safe_with_medium_severity_threat() {
        ...
        let has_high_or_critical = result.threats_detected.iter()
    ...
    fn test_check_steganography_returns_none_when_lsb_balanced() {
        ...
        let steg_threats: Vec<_> = result.threats_detected.iter()
            .filter(|t| matches!(t.threat_type, ThreatType::Steganography))
            .collect();
        assert!(steg_threats.is_empty(), "balanced LSB image must not trigger steganography");
    }
    ...
    fn test_check_steganography_none_all_levels() {
        ...
        for level in [SecurityLevel::Low, SecurityLevel::Medium, SecurityLevel::High, SecurityLevel::Maximum] {
            ...
            let steg_threats: Vec<_> = result.threats_detected.iter()
                .filter(|t| matches!(t.threat_type, ThreatType::Steganography))
                .collect();
            assert!(steg_threats.is_empty(), "balanced LSB must not trigger steganography at any level");
        }
    }
    ...
    fn test_validate_confidence_non_empty_threats() {
        ...
        let expected_avg = result.threats_detected.iter().map(|t| t.confidence).sum::<f32>()
    ...
    fn test_adversarial_check_called_for_all_security_levels() {
        ...
        for level in [SecurityLevel::Low, SecurityLevel::Medium, SecurityLevel::High, SecurityLevel::Maximum] {
            ...
            let adv: Vec<_> = result.threats_detected.iter()
                .filter(|t| matches!(t.threat_type, ThreatType::AdversarialPattern))
                .collect();
            assert!(adv.is_empty(), "uniform image must not trigger adversarial threat");
        }
    }
