...
impl GpuManager {
    ...
    pub fn get_info(&self) -> Result<GpuInfo> {
        ...
        for i in 0..device_count {
            let device = nvml
                .device_by_index(i)
                .context(format!("Failed to get device {}", i))?;
    ...
    pub fn collect_metal_stats(&self) -> Result<Vec<GpuStats>> {
        ...
        if let Ok(output) = output {
            ...
            let mem_cmd = Command::new("sysctl").arg("-n").arg("hw.memsize").output();
    ...
    pub fn get_metal_info_string(&self) -> Option<String> {
            ...
            .output()
        {
            if let Ok(cpu_info) = String::from_utf8(output.stdout) {
                ...
                // Get macOS version
                if let Ok(os_output) = Command::new("sw_vers").arg("-productVersion").output() {
...
mod tests {
    ...
    fn test_get_info_no_cuda_no_metal_returns_cpu_backend() {
        ...
        assert!(
            !info.available,
            "CPU backend should not be marked as available GPU"
        );
        assert_eq!(info.count, 0);
        assert!(info.devices.is_empty());
        assert!(matches!(info.backend, GpuBackend::Cpu));
