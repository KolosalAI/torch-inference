...
impl WhisperEngine {
    ...
    fn transcribe_fallback(&self, audio: &AudioData) -> Result<String> {
        ...
        let _avg_amplitude =
            audio.samples.iter().map(|s| s.abs()).sum::<f32>() / audio.samples.len() as f32;
        ...
        log::info!(
            "Audio analysis: duration={:.2}s, freq={:.1}Hz, est_words={}",
            duration,
            avg_frequency,
            estimated_words
        );
        ...
        Ok(format!(
            "[FALLBACK] Detected speech: {:.2}s duration, ~{} words, {:.1}Hz avg frequency",
...
mod tests {
    ...
    fn test_transcribe_contains_duration_info() {
        ...
        assert!(
            text.contains("1.00") || text.contains("1.0") || text.contains("duration"),
    ...
    fn test_transcribe_contains_words_estimate() {
        ...
        let text = engine.transcribe(&audio).unwrap();
        assert!(
            text.contains("words") || text.contains("~"),
    ...
    fn test_transcribe_file_with_valid_wav() {
        ...
        assert!(
            result.is_ok(),
            "transcribe_file should succeed on a valid WAV: {:?}",
            result
        );
        let text = result.unwrap();
        assert!(
            text.contains("[FALLBACK]"),
            "Expected fallback marker in: {}",
            text
        );
    }
    ...
    fn test_transcribe_file_missing_path_returns_error() {
        ...
        assert!(
            result.is_err(),
            "transcribe_file with missing path must error"
        );
    }
