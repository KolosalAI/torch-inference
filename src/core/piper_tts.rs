...
mod tests {
    ...
    fn test_piper_new_missing_model_path_returns_error() {
        ...
        assert!(
            msg.contains("model_path") || msg.contains("Missing"),
            "unexpected: {}",
            msg
        );
    }
    ...
    fn test_piper_new_missing_config_path_returns_error() {
        ...
        let msg = format!("{}", result.err().unwrap());
        assert!(
            msg.contains("config_path") || msg.contains("Missing") || msg.contains("not found"),
            "unexpected: {}",
            msg
        );
    }
    ...
    fn test_piper_synthesize_fallback_produces_audio() {
        ...
        assert!(
            result.is_ok(),
            "synthesize_fallback should not error: {:?}",
            result.err()
        );
