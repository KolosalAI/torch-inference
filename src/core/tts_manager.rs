...
use super::tts_engine::{EngineCapabilities, SynthesisParams, TTSEngine, TTSEngineFactory};
...
fn default_synthesis_cache_capacity() -> usize {
    128
}
...
impl TTSManager {
    ...
    pub async fn load_engine(
        ...
        // Warmup the engine
        engine.warmup().await.context("Engine warmup failed")?;
    ...
    pub async fn synthesize(
        ...
        log::info!(
            "Synthesizing with engine '{}': {} characters",
            engine_id,
            text.len()
        );
        let audio = engine
    ...
    pub async fn initialize_defaults(&self) -> Result<()> {
        ...
        match self
            .load_engine("kokoro-onnx".to_string(), "kokoro-onnx", kokoro_onnx_config)
            .await
        {
        ...
        match self
            .load_engine("kokoro".to_string(), "kokoro", kokoro_config)
            .await
        {
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            match self
                .load_engine("windows-sapi".to_string(), "windows-sapi", sapi_config)
                .await
            {
        ...
        match self
            .load_engine("vits".to_string(), "vits", vits_config)
            .await
        {
        ...
        match self
            .load_engine("styletts2".to_string(), "styletts2", styletts2_config)
            .await
        {
        ...
        match self
            .load_engine("bark".to_string(), "bark", bark_config)
            .await
        {
        ...
        match self
            .load_engine("xtts".to_string(), "xtts", xtts_config)
            .await
        {
        ...
        if engine_count == 0 {
            log::warn!(
                "[WARN]  No TTS engines loaded. Install models to enable TTS functionality."
            );
        } else {
...
mod tests {
    ...
    async fn test_load_demo_engine() {
        ...
        let result = manager
            .load_engine("test".to_string(), "demo", demo_config)
            .await;
    ...
    async fn test_synthesis() {
        ...
        let result = manager
            .synthesize("Hello, world!", Some("nonexistent"), params)
            .await;
    ...
    fn test_synthesis_cache_key_is_stable_across_calls() {
        ...
        assert_eq!(
            k1,
            TTSManager::synthesis_cache_key("Hello world", "kokoro-onnx", &params),
    ...
    fn test_synthesis_cache_key_distinguishes_inputs() {
        ...
        assert_ne!(
            k_text_a, k_text_b,
            "different text must produce different keys"
        );
        ...
        assert_ne!(
            k_engine_a, k_engine_b,
            "different engine must produce different keys"
        );

        let params_fast = SynthesisParams {
            speed: 2.0,
        ...
        assert_ne!(
            k_speed_a, k_speed_b,
            "different speed must produce different keys"
        );
    }
    ...
    fn test_with_engine_calls_closure_and_returns_result() {
        ...
        manager
            .engines
            .insert("mock".to_string(), Arc::new(MockEngine));

        let result: Option<String> = manager.with_engine("mock", |e| e.name().to_string());
        assert_eq!(
            result,
            Some("mock".to_string()),
            "closure should fire and return engine name"
        );
    }
    ...
    fn test_with_engine_returns_none_for_unknown_engine() {
        ...
        assert!(
            result.is_none(),
            "with_engine must return None for an unknown engine id"
        );
    }
    ...
    fn make_manager_with_mock(engine_id: &str) -> TTSManager {
        ...
        manager
            .engines
            .insert(engine_id.to_string(), make_mock_engine());
        manager
    }
    ...
    fn test_register_engine_duplicate_returns_err() {
        let manager = TTSManager::new(TTSManagerConfig::default());
        manager
            .register_engine("dup".to_string(), make_mock_engine())
            .unwrap();
    ...
    fn test_list_engines_returns_registered_names() {
        let manager = TTSManager::new(TTSManagerConfig::default());
        manager
            .register_engine("engine-a".to_string(), make_mock_engine())
            .unwrap();
        manager
            .register_engine("engine-b".to_string(), make_mock_engine())
            .unwrap();
    ...
    fn test_get_default_engine_some_after_registration() {
        ...
        manager
            .register_engine("the-default".to_string(), make_mock_engine())
            .unwrap();
    ...
    fn test_default_engine_round_trip_via_config() {
        ...
        manager
            .register_engine("alpha".to_string(), make_mock_engine())
            .unwrap();
        manager
            .register_engine("beta".to_string(), make_mock_engine())
            .unwrap();

        // default should be alpha
        assert_eq!(
            manager.get_default_engine().map(|e| e.name().to_string()),
            Some("mock".to_string())
        );
    ...
    async fn test_synthesize_uses_default_engine_when_none_given() {
        ...
        assert!(
            result.is_ok(),
            "synthesis with default engine should succeed: {:?}",
            result.err()
        );
    }
    ...
    fn test_get_stats_after_registration() {
        let manager = TTSManager::new(TTSManagerConfig::default());
        manager
            .register_engine("e1".to_string(), make_mock_engine())
            .unwrap();
        manager
            .register_engine("e2".to_string(), make_mock_engine())
            .unwrap();
        let stats = manager.get_stats();
    ...
    fn test_cache_key_voice_none_vs_some_differ() {
        let p_none = SynthesisParams {
            voice: None,
            ..SynthesisParams::default()
        };
        let p_some = SynthesisParams {
        ...
        assert_ne!(
            k1, k2,
            "voice=None vs voice=Some must produce different keys"
        );
    }
    ...
    fn test_cache_key_language_none_vs_some_differ() {
        ...
        assert_ne!(
            k1, k2,
            "language=None vs language=Some must produce different keys"
        );
    }

    #[test]
    fn test_cache_key_pitch_affects_key() {
        let p1 = SynthesisParams {
            pitch: 1.0,
            ..SynthesisParams::default()
        };
        let p2 = SynthesisParams {
            pitch: 0.5,
            ..SynthesisParams::default()
        };
    ...
    async fn test_synthesize_cache_grows_up_to_capacity() {
        ...
        for i in 0..4 {
            ...
            manager
                .synthesize(&text, Some("mock"), params.clone())
                .await
                .unwrap();
        }
        ...
        assert_eq!(
            stats.cache_size, 4,
            "cache should hold exactly 4 unique entries"
        );
    }
    ...
    async fn test_synthesize_text_too_long_returns_err() {
        ...
        assert!(
            result.is_err(),
            "text exceeding max_text_length should fail"
        );
        let msg = format!("{}", result.unwrap_err());
        assert!(
            msg.contains("1001") || msg.contains("too long") || msg.contains("1000"),
    ...
    fn test_synthesis_cache_key_separator_prevents_concatenation_collision() {
        ...
        assert_ne!(
            k1, k2,
            "NUL separator must prevent field-concatenation collisions"
        );
    }
    ...
    async fn test_synthesize_different_engines_have_independent_caches() {
        ...
        manager
            .engines
            .insert("mock1".to_string(), make_mock_engine());
        manager
            .engines
        ...
        assert_eq!(
            stats.cache_size, 2,
            "same text on two engines = 2 distinct cache entries"
        );
    ...
    async fn test_synthesize_none_engine_uses_config_default() {
        ...
        assert!(
            result.is_ok(),
            "None engine should use default: {:?}",
            result.err()
        );
    }
    ...
    async fn test_synthesize_with_voice_and_language_params() {
        ...
        let result = manager
            .synthesize("Hello with voice", Some("mock"), params)
            .await;
        assert!(
            result.is_ok(),
    ...
    async fn test_synthesize_different_params_produce_separate_cache_entries() {
        ...
        manager
            .synthesize("shared text", Some("mock"), params_a)
            .await
            .unwrap();
        manager
            .synthesize("shared text", Some("mock"), params_b)
            .await
            .unwrap();
        let stats = manager.get_stats();
        assert_eq!(
            stats.cache_size, 2,
            "different params must create separate cache entries"
        );
    }
    ...
    async fn test_synthesize_none_engine_missing_default_returns_err() {
        ...
        let msg = format!("{}", result.unwrap_err());
        assert!(
            msg.contains("not found") || msg.contains("kokoro-onnx"),
    ...
    async fn test_synthesize_cache_hit_does_not_grow_cache() {
        ...
        manager
            .synthesize("repeated", Some("mock"), params.clone())
            .await
            .unwrap();
        let stats_after_first = manager.get_stats();
        // Second call: hit — should NOT change cache_size
        manager
            .synthesize("repeated", Some("mock"), params)
            .await
            .unwrap();
        let stats_after_second = manager.get_stats();
        assert_eq!(
            stats_after_first.cache_size, stats_after_second.cache_size,
    ...
    async fn test_load_engine_torch_type_succeeds_and_registers() {
        ...
        let manager = TTSManager::new(TTSManagerConfig::default());
        let result = manager
            .load_engine("my-torch".to_string(), "torch", serde_json::json!({}))
            .await;
        assert!(
            result.is_ok(),
            "torch engine should load without error: {:?}",
            result.err()
        );
    ...
    async fn test_load_engine_torch_type_shows_in_list() {
        let manager = TTSManager::new(TTSManagerConfig::default());
        manager
            .load_engine("t1".to_string(), "torch", serde_json::json!({}))
            .await
            .unwrap();
    ...
    async fn test_initialize_defaults_runs_without_panic() {
        ...
        assert!(
            result.is_ok(),
            "initialize_defaults should not return Err: {:?}",
            result
        );
    ...
    async fn test_initialize_defaults_returns_ok_when_no_models_present() {
        ...
        assert!(
            result.is_ok(),
            "initialize_defaults must return Ok even when all engine loads fail: {:?}",
            result
        );
    }
    ...
    async fn test_initialize_defaults_is_idempotent_no_panic() {
        ...
        assert!(
            r1.is_ok(),
            "first initialize_defaults call should return Ok"
        );
        assert!(
            r2.is_ok(),
            "second initialize_defaults call should return Ok"
        );
    }
    ...
    async fn test_synthesize_cache_hit_returns_cloned_audio_data() {
        ...
        assert_eq!(
            audio_first.sample_rate, audio_second.sample_rate,
            "cached audio should have same sample_rate"
        );
        assert_eq!(
            audio_first.channels, audio_second.channels,
            "cached audio should have same channels"
        );
        assert_eq!(
            audio_first.samples.len(),
            audio_second.samples.len(),
        ...
        assert_eq!(
            stats.cache_size, 1,
            "same text and params must produce 1 cache entry"
        );
    }
    ...
    async fn test_synthesize_audio_duration_formula_is_sane() {
        ...
        assert!(
            duration_secs < 60.0,
            "duration should be less than 60 s for short text"
        );
    }
    ...
    async fn test_synthesize_empty_text_rejected_before_engine_call() {
        ...
        assert!(
            result.is_err(),
            "empty text must be rejected by validate_text"
        );
    }
    ...
    async fn test_initialize_defaults_covers_all_engine_load_arms() {
        ...
        assert!(
            result.is_ok(),
            "initialize_defaults must return Ok even when some engines fail: {:?}",
            result
        );
    }
    ...
    async fn test_initialize_defaults_engine_count_branch_line_309() {
        ...
        assert!(
            stats.total_engines < usize::MAX,
            "engine count must be a finite number: {}",
            stats.total_engines
        );
    }
    ...
    async fn test_initialize_defaults_non_zero_engine_count_line_313() {
        ...
        assert!(
            stats.total_engines <= 10,
            "should not exceed the number of engines attempted: {}",
            stats.total_engines
        );
    }
    ...
    async fn test_initialize_defaults_kokoro_block_lines_212_219() {
        ...
        assert!(
            manager.get_engine("kokoro").is_some(),
    ...
    async fn test_initialize_defaults_vits_block_lines_262_270() {
        ...
        assert!(
            manager.get_engine("vits").is_some(),
    ...
    async fn test_initialize_defaults_styletts2_block_lines_274_282() {
        ...
        manager.initialize_defaults().await.unwrap();
        assert!(
            manager.get_engine("styletts2").is_some(),
    ...
    async fn test_initialize_defaults_bark_block_lines_286_294() {
        ...
        manager.initialize_defaults().await.unwrap();
        assert!(
            manager.get_engine("bark").is_some(),
    ...
    async fn test_initialize_defaults_xtts_block_lines_298_306() {
        ...
        manager.initialize_defaults().await.unwrap();
        assert!(
            manager.get_engine("xtts").is_some(),
    ...
    async fn test_initialize_defaults_return_ok_and_stats_consistent() {
        ...
        let stats = manager.get_stats();
        assert_eq!(
            stats.engine_ids.len(),
            stats.total_engines,
            "engine_ids length must equal total_engines"
        );
        assert_eq!(
            stats.cache_capacity, 128,
    ...
    async fn test_initialize_defaults_second_call_covers_err_arms() {
        ...
        assert!(
            result.is_ok(),
            "second initialize_defaults call must still return Ok: {:?}",
            result
        );
    ...
    async fn test_initialize_defaults_completes_without_panic() {
        ...
        assert!(
            result.is_ok(),
            "initialize_defaults should succeed even if engines fail: {:?}",
            result
        );
    }
    ...
    async fn test_synthesize_returns_err_when_engine_not_found() {
        ...
        assert!(
            ...
            "error should mention engine id or 'not found': {}",
            msg
        );
    }
    ...
    async fn test_synthesize_cache_miss_then_hit() {
        ...
        let first = manager
            .synthesize("hello world", Some("mock"), params.clone())
            .await;
        assert!(
            first.is_ok(),
        ...
        let second = manager
            .synthesize("hello world", Some("mock"), params)
            .await;
        assert!(
            second.is_ok(),
