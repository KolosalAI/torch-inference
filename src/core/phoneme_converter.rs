...
impl EnhancedG2P {
    ...
    pub(crate) fn ipa_to_tokens(ipa: &str, converter: &PhonemeConverter) -> Vec<i64> {
        let mut tokens = Vec::new();
        let mut chars = ipa.chars().peekable();

        while let Some(ch) = chars.next() {
            // Try two-character IPA digraph first (e.g. diphthongs), then single character.
            let phoneme = if let Some(&next) = chars.peek() {
                let two_char = format!("{}{}", ch, next);
                if converter.vocab.contains_key(&two_char) {
...
mod tests {
    ...
    fn test_enhanced_g2p_all_dictionary_words_produce_tokens() {
        let g2p = EnhancedG2P::new().unwrap();
        let words = [
            "the", "quick", "brown", "fox", "test", "hello", "world", "one", "two", "three",
        ];
        for word in words {
            let tokens = g2p.text_to_phonemes(word).unwrap();
            assert!(
                !tokens.is_empty(),
                "No tokens for dictionary word '{}'",
                word
            );
        }
    }
    ...
    #[test]
    fn test_ipa_to_tokens_digraph_matched_before_single() {
        // Verify the greedy 2-char path in ipa_to_tokens fires when a digraph
        // entry exists in the vocab.  Uses a synthetic mini-vocab so the test
        // is independent of Kokoro's real token IDs.
        let mut vocab = HashMap::new();
        vocab.insert("a".to_string(), 1i64);
        vocab.insert("b".to_string(), 2i64);
        vocab.insert("ab".to_string(), 99i64); // digraph wins over two singles
        let converter = PhonemeConverter { vocab };

        // "ab" → should match digraph (token 99), not two singles [1, 2]
        let tokens = EnhancedG2P::ipa_to_tokens("ab", &converter);
        assert_eq!(
            tokens,
            vec![99],
            "digraph 'ab' should produce a single token 99, not [1, 2]"
        );

        // "a" alone → single-char fallback
        let tokens = EnhancedG2P::ipa_to_tokens("a", &converter);
        assert_eq!(tokens, vec![1]);

        // "ba" → 'b' peeks at 'a', "ba" not in vocab → falls back to [2, 1]
        let tokens = EnhancedG2P::ipa_to_tokens("ba", &converter);
        assert_eq!(tokens, vec![2, 1]);
    }

    #[test]
    fn test_ipa_to_tokens_digraph_does_not_consume_too_many_chars() {
        // After a digraph match, the char after it must still be processed normally.
        let mut vocab = HashMap::new();
        vocab.insert("ab".to_string(), 99i64);
        vocab.insert("c".to_string(), 3i64);
        let converter = PhonemeConverter { vocab };

        // "abc" → "ab" consumed as digraph (99), then "c" (3)
        let tokens = EnhancedG2P::ipa_to_tokens("abc", &converter);
        assert_eq!(tokens, vec![99, 3]);
    }
    ...
    fn test_word_to_phonemes_vowel_i_line_126() {
        ...
        assert!(
            tokens.contains(&102),
            "token for 'ɪ' (ID 102) should appear for 'i'"
        );
    }
    ...
    fn test_word_to_phonemes_vowel_u_line_128() {
        ...
        assert!(
            tokens.contains(&135),
            "token for 'ʊ' (ID 135) should appear for 'u'"
        );
    }
