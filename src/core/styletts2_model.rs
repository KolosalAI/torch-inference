...
pub fn duration_expand(hidden: &Tensor, durations: &Tensor) -> Tensor {
    debug_assert_eq!(
        hidden.size()[0],
        1,
        "duration_expand only supports batch_size=1"
    );
...
impl StyleTTS2Inference {
    pub fn new(safetensors_path: &std::path::Path, device: Device) -> Result<Self> {
        ...
        if !unmatched.is_empty() {
            log::warn!(
                "Safetensors file contained {} keys not registered in model: {:?}",
                unmatched.len(),
    ...
    pub fn tokens_to_mel(&self, tokens: &[i64], speaker: Option<i64>) -> Result<Vec<Vec<f32>>> {
        tch::no_grad(|| {
            let tok_tensor = Tensor::of_slice(tokens).to_device(self.device).unsqueeze(0); // [1, seq]

            let hidden = self.text_encoder.forward(&tok_tensor); // [1, seq, 512]
            let durations = self.dur_predictor.forward(&hidden); // [1, seq]
            let style = self
                .speaker_embed
                .forward(speaker.unwrap_or(0), self.device); // [1, 128]
...
mod tests {
    ...
    fn test_styletts2_inference_new_errors_without_torch() {
        ...
        assert!(
            msg.contains("torch") || msg.contains("StyleTTS2"),
            "unexpected: {}",
            msg
        );
    }
