...
impl InstanceMetrics {
    ...
    fn load_score(&self) -> u64 {
        ...
        // Previously weight=1000 made latency nearly irrelevant to routing decisions.
        active * 500 + latency
    }
}
...
impl<T> ModelPool<T> {
    ...
    pub fn add_instance(&self, model_name: String, instance: Arc<T>) {
        ...
        if instances.len() < self.max_instances_per_model {
            ...
            info!(
                "Added model instance for '{}', total: {}",
                model_name,
                instances.len()
            );
        }
    ...
    pub async fn acquire(
        &self,
        model_name: &str,
    ) -> Option<(Arc<T>, Arc<InstanceMetrics>, Instant)> {
        let semaphore = self
            .semaphores
        ...
        debug!(
            "Acquired model instance '{}' (instance {}, load score: {})",
            model_name,
            idx,
            wrapper.metrics.load_score()
        );
    ...
    pub fn instance_count(&self, model_name: &str) -> usize {
        self.instances.get(model_name).map(|v| v.len()).unwrap_or(0)
    }
    ...
    pub fn get_stats(&self) -> ModelPoolStats {
        ...
        let total_instances: usize = self.instances.iter().map(|entry| entry.value().len()).sum();
...
mod tests {
    ...
    async fn test_acquire_returns_none_when_instances_vec_is_empty() {
        ...
        assert!(
            result.is_none(),
            "expected None when instance list is empty"
        );
    }
