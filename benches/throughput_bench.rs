    ...
    for (label, chars) in [
        ...
        group.bench_with_input(BenchmarkId::new("batch_queue", label), &text, |b, text| {
            ...
            b.iter(|| {
                ...
                black_box(processor.add_request(req)).ok();
...
fn bench_tts_throughput(c: &mut Criterion) {
    ...
    let cases: &[(&str, &str)] = &[
        ("short_54", "The quick brown fox jumps over the lazy dog."),
        (
            "sentence_121",
...
fn bench_image_preprocessing_throughput(c: &mut Criterion) {
    ...
    for (label, w, h) in sizes {
        ...
        let src: DynamicImage =
            DynamicImage::ImageRgb8(ImageBuffer::<Rgb<u8>, _>::from_fn(w * 4, h * 4, |x, y| {
                Rgb([(x % 255) as u8, (y % 255) as u8, ((x + y) % 255) as u8])
            }));

        group.bench_with_input(BenchmarkId::new("resize_to", label), &src, |b, src| {
            b.iter(|| black_box(src.resize_exact(*w, *h, image::imageops::FilterType::Lanczos3)));
        });
    }
    group.finish();
}
        ...
        group.bench_with_input(
            ...
            |b, payload| {
                ...
                b.iter(|| {
                    ...
                    cache.set(key.clone(), black_box(payload.clone()), 300).ok();
            ...
            |b, &batch_size| {
                ...
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        black_box(processor.try_form_batch())
                    });
                });
            },
        );
