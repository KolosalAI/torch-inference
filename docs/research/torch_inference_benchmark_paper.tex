...
On GPU configurations, the performance landscape shifts fundamentally as compute-bound operations dominate over framework overhead (\cref{tab:latency_gpu}). The latency gap between frameworks narrows because GPU kernel execution time---which is largely framework-independent---becomes the primary bottleneck rather than host-side orchestration.
...
\Cref{tab:sota} presents benchmark results across representative models from each architecture family, enabling practitioners to assess performance characteristics across the model complexity spectrum:
...
\begin{enumerate}
    ...
    \item \textbf{\timesX{22.7}} faster cold-start model loading (\SI{108}{\milli\second} vs.\ \SI{2.5}{\second})
...
\textbf{Key trade-offs and deployment guidance:} TensorRT with INT8 quantization achieves \textbf{\timesX{6} higher GPU throughput} than our framework (\num{2500}\fpssuffix{} vs.\ \num{417}\fpssuffix{}), making it unambiguously superior for maximum-throughput production GPU deployments. Kolosal Torch-Inference's advantages are most pronounced for specific deployment scenarios: (a)~serverless architectures where cold-start latency directly impacts billing and user experience, (b)~development and A/B testing pipelines requiring frequent model switching, (c)~memory-constrained edge deployments where the \timesX{2.86} memory reduction enables deployment on smaller instances, and (d)~organizations prioritizing deployment simplicity and maintainability over peak performance.

\textbf{Limitations and scope:} Primary validation was conducted on Apple Silicon (M2 Pro, MPS backend); while NVIDIA GPU baseline results confirm CUDA acceleration (up to \timesX{14.4} speedup for Vision Transformers), production GPU deployments should be independently validated. Transformer architectures show only \SIrange{8}{15}{\percent} improvement versus \SIrange{30}{50}{\percent} for CNNs, reflecting the compute-bound nature of attention mechanisms that cannot be addressed by framework-level optimizations. Native Rust ML frameworks (Candle, Burn) were not benchmarked as they require model reimplementation rather than loading TorchScript checkpoints, representing a distinct deployment paradigm.

The complete benchmark suite, raw data, and source code are available at \url{https://github.com/kolosal-ai/torch-inference} to enable reproducibility and independent validation.
