...
\vspace{0.2em}
\noindent \textbf{Results.} Relative to TorchServe: \timesX{1.57} lower P95 latency ($p < 0.001$, $d = 1.54$); \timesX{2.86} smaller memory (\SI{420}{\mega\byte} vs.\ \SI{1200}{\mega\byte}); \timesX{22.7} faster cold-start (\SI{108}{\milli\second} vs.\ \SI{2.5}{\second}). Memory estimation MAPE: \SI{0.84}{\percent}. Throughput spans \numrange{3}{962}\fpssuffix{} across model complexity. However, TensorRT INT8 achieves \timesX{6} higher GPU throughput (\num{2500} vs.\ \num{417}\fpssuffix{}).
...
\vspace{0.5em}
\noindent\textit{Note.} Baseline PyTorch CUDA performance without TensorRT optimization. Hardware: Intel i7-12700H (14 cores, 20 threads), 64GB DDR4, RTX 3060 Laptop GPU (6GB VRAM), Windows 11 Pro. PyTorch 2.8.0+cu118, batch size 1, 100 iterations after 20 warmup iterations. ``vs CPU'' column shows speedup factor compared to CPU-only inference on same hardware.
\end{table}
...
This paper has presented Kolosal Torch-Inference, a Rust-based inference framework designed to address the systemic inefficiencies of Python-based model serving solutions. Through comprehensive benchmarking against 14 deployment frameworks across 55 image classification architectures ($N = 144$ configurations), we have quantified the performance trade-offs between deployment simplicity, latency, memory efficiency, and peak throughput.
...
\begin{enumerate}
    ...
    \item Throughput spanning \numrange{3}{962}\fpssuffix{} across model complexity, from SqueezeNet to EfficientNetV2-L
\end{enumerate}

\textbf{Trade-offs and positioning.} TensorRT with INT8 quantization achieves \textbf{\timesX{6} higher GPU throughput} (\num{2500}\fpssuffix{} vs.\ \num{417}\fpssuffix{}), making it unambiguously superior for maximum-throughput production GPU workloads. Kolosal Torch-Inference occupies a distinct niche: scenarios where deployment velocity, cold-start performance, and operational simplicity outweigh peak throughput---including serverless functions, development pipelines, A/B testing infrastructure, and memory-constrained edge devices.

\textbf{Scope and limitations.} Primary validation was conducted on Apple Silicon; NVIDIA GPU results require independent production validation. Transformer architectures show only \SIrange{8}{15}{\percent} improvement versus \SIrange{30}{50}{\percent} for CNNs, reflecting compute-bound attention mechanisms that cannot be optimized at the framework level. Native Rust frameworks (Candle, Burn) were not evaluated as they require model reimplementation.
